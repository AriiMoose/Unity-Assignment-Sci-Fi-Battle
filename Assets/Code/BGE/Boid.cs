...
#region Integration
        ...
        private Vector3 CalculateWeightedPrioritised()
        {
            ...
            if (fleeEnabled)
            {
                ...
                // If the current object is the Destiny's Ascension and is not currently fleeing
                else if (gameObject.tag == "DA" && fleeTarget == null)
                {
                    fleeTarget = GameObject.FindGameObjectWithTag("Sovereign");

                    // If no object with "Sovereign" tag is found, try "Geth"
                    if (fleeTarget == null)
                    {
                        fleeTarget = GameObject.FindGameObjectWithTag("Geth");
                    }
                }
